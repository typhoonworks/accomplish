defmodule AccomplishWeb.Plugs.AuthorizeAPIKey do
  @moduledoc """
  Plug for authorizing access to the public API using API keys.
  ...
  def call(conn, _opts) do
    ...
    with {:ok, authorization} <- get_authorization_header(conn),
      ...
      {:error, :unsupported_auth_scheme} ->
        Helpers.unauthorized(conn, "Unsupported authentication scheme. Only Basic is supported.")

      {:error, :invalid_auth_header} ->
        Helpers.unauthorized(conn, "Invalid authorization header format.")

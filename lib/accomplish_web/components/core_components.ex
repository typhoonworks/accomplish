defmodule AccomplishWeb.CoreComponents do
  @moduledoc """
  ...
  def header(assigns) do
    ...
    <header class={[@actions != [] && "flex items-center justify-between gap-6", @class]}>
      <div>
        <h1 class="text-lg font-semibold leading-8 text-zinc-50">
          {render_slot(@inner_block)}
        </h1>
        <p :if={@subtitle != []} class="mt-2 text-sm leading-6 text-zinc-100">
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(AccomplishWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(AccomplishWeb.Gettext, "errors", msg, opts)
    end
  end
